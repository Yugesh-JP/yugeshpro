...
headers = ['DomainName', 'web_domains']

if not os.path.exists(r'VendorNames_Websites Output.csv'):
    with open('VendorNames_Websites Output', 'a+', newline="") as file:
...
with open(('VendorNames_Websites'), "r", encoding="utf8") as f:
    reader = csv.reader(f)
    next(reader)
    for col in reader:
        try:
            ...
            for cite in soup.find('cite', {'class': "iUh30 Zu0yb tjvcx"}):
             print(company, cite)
             with open('VendorNames_Websites Output.csv', 'a+', newline="", encoding='utf-8') as file:
                writer = csv.DictWriter(file, fieldnames=headers)
                writer.writerow({'DomainName': Company,
                         'Web_domains': Cite,
                                 })
